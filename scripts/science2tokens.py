...
def from_texts_to_tokens(df):
    ...
    for ngram in range(1, 3):
        ...
        vectorizer = TfidfVectorizer(stop_words=stopwords.words('english'), token_pattern=r'\b[a-zA-Z]{3,}\b',
                                     min_df=0.001, max_df=0.75, ngram_range=(ngram, ngram))
        with open(f'../tokens/abstract_tokens_ngrams_{ngram}.txt', 'w') as abstract_f, \
                open(f'../tokens/title_tokens_ngrams_{ngram}.txt', 'w') as title_f:
...
NAME_DF = '../datasets nature/nature_computer_science.csv'
